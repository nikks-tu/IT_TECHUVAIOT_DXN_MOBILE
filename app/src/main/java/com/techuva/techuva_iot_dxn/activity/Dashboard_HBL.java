...
public class Dashboard_HBL extends BaseActivity implements MyFirebaseMessagingService.EventListener {
    ...
    private void Init_Views() {
       ...
       /* if(productName.equals("HBL"))
        ...
        FirebaseInstanceId.getInstance().getInstanceId().addOnCompleteListener(task -> {
            ...
            serviceCallForSavingToken(token);Log.d("Device ID", token);
    ...
    public void goto_home() {
        ...
        {
            intent = new Intent(context, Dashboard_HBL.class);
            startActivity(intent);
        }
    ...
    private void generateDataList(List<CurrentDataValueObject> values)
    {
        ...
        if(arrayList.size()>0)
        {
            gridAdapterDashboard = new GridAdapterDashboard(Dashboard_HBL.this, R.layout.grid_item_dashboard, arrayList);
            grid_bottom.setAdapter(gridAdapterDashboard);
        }
    }

    public void showLoaderNew() {
        runOnUiThread(new Dashboard_HBL.Runloader(getResources().getString(R.string.loading)));
    }
